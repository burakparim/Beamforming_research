...
car_fw = 2*1j*1*exp(-1j*omega*(T/2))*sin(k*d*(1 + cos(theta))/2);
car_bw = 2*1j*1*exp(-1j*omega*(T/2))*sin(k*d*(1 - cos(theta))/2);
%Plot directional response over angles. 
figure;
polar(theta,abs(car_fw),'b');
% title('Directivity pattern of forward cardioid')
% ylabel('response in dB')
% figure;
hold on
polar(theta,abs(car_bw));
